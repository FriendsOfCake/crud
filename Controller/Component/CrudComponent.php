...
class CrudComponent extends Component {
	...
	*/
	...
	protected function addAction() {
		if ($this->request->is('post')) {
			...
			if ($this->model->saveAll($this->request->data, array('validate' => 'first', 'atomic' => true))) {
				$this->setFlash(sprintf('Succesfully created %s', Inflector::humanize($this->modelName)), 'success');
				...
				$this->setFlash(sprintf('Could not create %s', Inflector::humanize($this->modelName)), 'error');
	...
	protected function editAction($id = null) {
		...
		if ($this->request->is('put')) {
			...
			if ($this->model->saveAll($this->request->data, array('validate' => 'first', 'atomic' => true))) {
				$this->setFlash(sprintf('%s was succesfully updated', ucfirst(Inflector::humanize($this->modelName))), 'success');
				$this->eventManager->dispatch(new CakeEvent('Crud.afterSave', $subject = $this->getSubject(array('id' => $id, 'success' => true))));
				$this->redirect($subject, array('action' => 'index'));
			} else {
				$this->setFlash(sprintf('Could not update %s', Inflector::humanize($this->modelName)), 'error');
			...
			if (empty($this->request->data)) {
				...
				$this->setFlash(sprintf('Could not find %s', Inflector::humanize($this->modelName)), 'error');
				$this->redirect($subject, array('action' => 'index'));
			}
	...
	protected function deleteAction($id = null) {
		...
		if ($event->isStopped()) {
			$this->setFlash(sprintf('Could not delete %s', Inflector::humanize($this->modelName)), 'error');
			$this->redirect($subject, array('action' => 'index'));
		}

		if ($this->request->is('delete')) {
			if ($this->model->delete($id)) {
				$this->setFlash(sprintf('Successfully deleted %s', Inflector::humanize($this->modelName)), 'success');
			...
			$this->setFlash(sprintf('Invalid HTTP request', Inflector::humanize($this->modelName)), 'error');
		}
	...
	*/
	protected function setFlash($message, $element = 'default', $params = array(), $key = 'flash') {
		$this->eventManager->dispatch(new CakeEvent('Crud.setFlash', $subject = $this->getSubject(compact('message', 'element', 'params', 'key'))));
