...
<?php if (isset($sidebarLinks) && $sidebarLinks !== false) : ?>
  <div class="actions">
    <h3><?php echo __d('cake', 'Actions'); ?></h3>
    <ul>
    <?php if (is_array($sidebarLinks)) : ?>
      <?php
        foreach ($sidebarLinks as $sidebarLink) {
          echo "\t\t<li>";
          if ($sidebarLink['type'] == 'post') {
            echo $this->Form->postLink($sidebarLink['title'], $sidebarLink['url'], $sidebarLink['options'], $sidebarLink['confirmMessage']);
          } else {
            echo $this->Html->link($sidebarLink['title'], $sidebarLink['url'], $sidebarLink['options'], $sidebarLink['confirmMessage']);
          }
          echo " </li>\n";
        }
      ?>
